...
export const PIPELINE_STEPS = [
  ...
  { id: 'module_01', label: '01: Auditoria de Receitas', group: 'modules' },
  { id: 'module_02', label: '02: Conformidade Legal', group: 'modules' },
  { id: 'module_03', label: '03: Auditoria de Despesas', group: 'modules', lite_skip: true },
  { id: 'module_04', label: '04: Consistência Financeira', group: 'modules' },
  { id: 'module_05', label: '05: Qualidade Documental', group: 'modules' },
  { id: 'module_06', label: '06: Parecer Final', group: 'modules' },
  { id: 'module_07', label: '07: Auditoria Quantitativa', group: 'modules', lite_skip: true },
  { id: 'csv', label: 'CSV de inconsistências', group: 'post' },
  ...
  const handleProgress = useCallback((p) => {
    setStepStates(prev => {
      ...
      if (p.current_step) {
        ...
        if (!lastStepIdRef.current) {
          // Primeiro evento: marca steps anteriores como done
          const idx = PIPELINE_STEPS.findIndex(s => s.id === stepId);
          if (idx > 0) {
            for (let i = 0; i < idx; i++) {
              next[PIPELINE_STEPS[i].id] = 'done';
            }
          }
        } else if (lastStepIdRef.current !== stepId) {
