"""
Gerador de Parecer Técnico Oficial em PDF de Auditoria Condominial.
Utiliza ReportLab para compor um documento formal e visualmente polido.
"""
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_footer(self, page_count):
        ...
        # Texto do rodapé
        self.drawString(40, 22, "AnaliseContas v7.0 — Sistema Automatizado de Auditoria Condominial")
...
def generate_official_pdf_report(output_dir: Path, pdf_name: str, domain: str = "cs") -> bool:
    ...
    pdf_target = output_dir / "parecer_auditoria_oficial.pdf"
    ...
    # Cabeçalho Oficial
    story.append(Paragraph("PARECER TÉCNICO DE AUDITORIA CONDOMINIAL", title_style))
    story.append(Paragraph("ANÁLISE CRÍTICA DE CONTAS · AUDITORIA FINANCEIRA", subtitle_style))
    ...
    domain_map = {
        "res": "Condomínio Residencial",
        "com": "Condomínio Comercial / Corporativo",
        "mis": "Condomínio Misto (Residencial + Comercial)",
    }
    ...
    meta_table_data = [
        ...
        [
            ...
            Paragraph("AnaliseContas v7.0", body_style),
        ],
    ]
    ...
    modules_summary = [
        ...
        ["00", "Estrutura do Documento", "Conformidade formal e organização", "Auditado"],
        ["01", "Receitas", "Receitas condominiais, fundos, multas", "Auditado"],
        ["02", "Conformidade Legal", "Lei 4.591/64, atas, tributação", "Auditado"],
        ["03", "Despesas", "Legitimidade, proporcionalidade, documentação", "Auditado"],
        ["04", "Consistência Financeira", "Cruzamento números, rateios, saldos", "Auditado"],
        ["05", "Qualidade Documental", "Clareza, terminologia, informações ausentes", "Auditado"],
        ["07", "Auditoria Quantitativa", "Totais, médias, rateios matemáticos", "Auditado"],
    ]
    # Bibliography audit not applicable for condominium accounts — skip bib row
    ...
    # Campo de Assinatura do Auditor
    story.append(KeepTogether([
        Paragraph("4. Declaração do Auditor", section_heading),
        Paragraph(
            "O auditor abaixo assinado atesta a realização da análise de contas "
            "e emite o parecer técnico aqui apresentado.",
            body_style,
        ),
        Spacer(1, 30),
        Table([
            [
                Paragraph("__________________________________________<br/><b>Auditor Responsável</b><br/><font size=7 color='#6B7280'>Contador / Auditor</font>", ParagraphStyle('Sig1', parent=body_style, alignment=1)),
                Paragraph("__________________________________________<br/><b>Revisor</b><br/><font size=7 color='#6B7280'>Auditor 2</font>", ParagraphStyle('Sig2', parent=body_style, alignment=1)),
            ]
        ], colWidths=[250, 250]),
    ]))
