...
try:
    ...
    # Fallback se bleach não estiver instalado
    import warnings
    warnings.warn("bleach não instalado — usando sanitização regex (menos seguro). pip install bleach")

    def sanitize_html(raw_html: str) -> str:
        """Fallback regex-based — NÃO recomendado para produção."""
        if not raw_html:
            return ""
        cleaned = re.sub(
            r"<\s*/?\s*(script|style|iframe|object|embed|link|meta|base|form|input|button|svg|math)\b[^>]*>",
            "", raw_html, flags=re.IGNORECASE,
        )
        cleaned = re.sub(r'\bon\w+\s*=\s*["\'][^"\']*["\']', "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r'\bon\w+\s*=\s*\S+', "", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r'(?:href|src|action)\s*=\s*["\']?\s*javascript\s*:', "", cleaned, flags=re.IGNORECASE)
        return cleaned
...
try:
    ...
    import warnings
    warnings.warn("bcrypt não instalado — autenticação usa comparação direta (menos seguro). pip install bcrypt")
