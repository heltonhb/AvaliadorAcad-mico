...
export default function Results() {
  ...
  // Encontrar score.json para nota final
  const scoreFile = analysisDetail?.files?.find(f => f.name === 'score.json');
  ...
  if (analyses.length === 0) {
    return (
      <div className="animate-fade-in text-center py-16 sm:py-24">
        ...
        <p className="text-[var(--text-tertiary)] text-sm max-w-md mx-auto mb-8 leading-relaxed">
          Os resultados das análises aparecem aqui. Faça o upload de um PDF acadêmico
    ...
    <div className="animate-fade-in space-y-6">
      ...
      {analysisDetail && (
        <>
          {/* Score Card */}
          {scoreData && (
            <div className="p-6 rounded-2xl bg-gradient-to-br from-teal-500/20 via-emerald-500/10 to-transparent border border-teal-500/30 animate-fade-in">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="text-center sm:text-left">
                  <div className="flex items-center gap-4">
                    <div className="text-5xl font-extrabold text-[var(--accent)]">{scoreData.score || scoreData.nota || '--'}</div>
                    <div>
                      <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider">Nota da Banca</p>
                      <p className="text-lg font-bold text-[var(--text-primary)]">{scoreData.decision || 'Pendente'}</p>
                    </div>
                  </div>
                  {scoreData.contribution_level && (
                    <p className="text-sm text-[var(--text-secondary)] mt-2">
                      Nível de Contribuição: <span className="font-semibold text-[var(--accent)]">{scoreData.contribution_level}</span>
                    </p>
                  )}
                </div>
                <button
...
/* ===== Rich Markdown Report Card (Lazy Loaded) ===== */
function RichReportCard({ file, analysisId }) {
  ...
  // Lazy load: carrega quando está visível pela primeira vez
  useEffect(() => {
    if (open && !hasLoaded) {
      loadContent();
    }
  ...
  return (
    <div className="rounded-xl border border-[var(--border-subtle)] bg-white/[0.01] overflow-hidden">
      <button
        ...
        aria-expanded={open}
        aria-label={`${open ? 'Fechar' : 'Abrir'} relatório ${file.name}`}
      >
        ...
        <span className={`text-[var(--text-muted)] transition-transform duration-200 ${open ? 'rotate-180' : ''}`} aria-hidden="true">▼</span>
      </button>
...
function PresentationsTab({ files, selectedId }) {
  ...
  const officialPdf = files.find(f => f.name === 'parecer_banca_oficial.pdf');
  const slidePdfs = files.filter(f => f.name !== 'parecer_banca_oficial.pdf');
      ...
      {officialPdf && (
        <div className="p-5 rounded-xl border border-teal-500/30 bg-gradient-to-r from-teal-500/10 via-emerald-500/5 to-transparent flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="badge badge-teal font-semibold">Oficial da Banca</span>
              ...
              <p className="text-xs text-[var(--text-tertiary)]">Pronto para impressão e assinatura da comissão examinadora</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <a
