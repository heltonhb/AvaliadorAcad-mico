    ...
    <div className="animate-fade-in space-y-6">
      <div>
        <h2 className="text-xl font-bold text-[var(--text-primary)] mb-1 flex items-center gap-2">
          ...
          Comparativo de Versões (V1 vs. V2 / Bancas)
        </h2>
        <p className="text-sm text-[var(--text-tertiary)]">
          Compare a evolução entre versões do mesmo trabalho ou o desempenho de diferentes candidatos.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-4">
          <label className="block text-xs font-semibold text-[var(--text-tertiary)] uppercase mb-2">
            1️⃣ Versão Base (ex: V1 / Qualificação)
          </label>
          <select
            value={baseId}
        ...
        <div className="card p-4">
          <label className="block text-xs font-semibold text-[var(--text-tertiary)] uppercase mb-2">
            2️⃣ Versão Comparada (ex: V2 / Defesa Final)
          </label>
          <select
            value={targetId}
      ...
      {comparison && !loading && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Base Card */}
            <div className="card p-5 space-y-4 bg-white/[0.01]">
              <div className="flex items-center justify-between pb-3 border-b border-[var(--border-subtle)]">
                <div>
                  <span className="text-xs text-[var(--text-muted)] font-mono uppercase tracking-wider">Versão Base (V1)</span>
                  <h4 className="text-base font-bold text-[var(--text-primary)]">{comparison.base.name}</h4>
                </div>
                <div className="text-right">
                  <span className="text-3xl font-extrabold text-[var(--text-secondary)]">
                    {comparison.base.nota !== null ? `${comparison.base.nota}` : '--'}
                  </span>
            ...
            {/* Target Card */}
            <div className="card p-5 space-y-4 border-[var(--accent-border)] bg-gradient-to-br from-teal-500/5 to-transparent">
              <div className="flex items-center justify-between pb-3 border-b border-[var(--border-subtle)]">
                <div>
                  <span className="text-xs text-[var(--accent)] font-mono uppercase tracking-wider">Versão Atualizada (V2)</span>
                ...
                <div className="text-right">
                  <span className="text-3xl font-extrabold text-[var(--accent)]">
                    {comparison.target.nota !== null ? `${comparison.target.nota}` : '--'}
                  </span>
                  <span className="text-sm text-[var(--text-muted)]">/10</span>
                </div>
              </div>
